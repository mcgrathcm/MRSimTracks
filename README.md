...
| Function | Purpose |
|---|---|
| `load_flow(path, active_key=..., mesh_mode="auto")` | Load one `.vtu` with time-labeled fields, a `.pvd`, a directory, or a VTU list. Static geometry is stored once; moving coordinates and changed topologies are retained only when needed. Declare `mesh_mode="static"`, `"moving"`, or `"changing_topology"` to skip automatic classification. `subsamp=N` keeps every Nth frame; `center_mesh=True` applies one frame-0-based translation to all mesh frames before sampling. |
| `load_ale_flow(path, velocity_scale=...)` | Load physical velocity, mesh velocity, and displacement on a verified static reference mesh for tracking on its deformed states. `center_mesh=True` applies one frame-0-based translation to all deformed mesh states before sampling. |
| `load_mesh_motion(path, displacement_key=None)` | Load fixed-topology nodal motion from moved-node VTUs or a three-component displacement field, then seed and evaluate attached material particles without velocity integration. `center_mesh=True` applies one frame-0-based translation to all mesh frames before seeding. `trajectory(..., output_path=..., pbar=True)` streams positions and exact times to HDF5 with frame progress reporting. |
